...
such).
  ...
  that concatenation occurs in linear across various implementations.
...
best to implement all six operations (__eq__, __ne__, __lt__, __le__,
__gt__, __ge__) rather than relying on other code only exercise
particular comparison.
  ...
  the interpreter may swap y > x with x < y, y >= x with x <= y, and
  x != y. The sort() and min() operations are guaranteed to use the <
  operator and the max() function use the > operator. However, it is
  best to implement all six operations so that confusion doesn't arise
  in other contexts.
...
binds a lambda expression directly to an identifier.
  ...
  No:
   f = lambda x: 2*x
  The first form means that the name of the resulting function objects
  ...
  can be embedded inside a large expression)

* Derive exceptions from Exception rather than BaseException. Direct
inheritance from BaseException is reserved for exception where
catching them is almost always the wrong thing to do.
  ...
  suffix "Error" to your exceptions classes if the exception is an
...
* When raising an exception in Python 2, use raise ValueError('
message') instead of the older form raise ValueError, 'message'.
...
possible instead of using a bare except: clause.
  ...
  A good rule of thumb is to limit use of bare 'except' clauses two
  cases:
...
binding syntax added in Python 2.6:
  try:
      ...
      raise DataProcessingFaileError(str(exc))
...
* Additionally, for all try/except clause, limit the try clause to the
...
bugs.
  ...
  No:
   try:
       # Too broad!
       return handle_value(collecton[key])
   except KeyError:
